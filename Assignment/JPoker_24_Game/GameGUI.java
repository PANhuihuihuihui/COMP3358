...
public class GameGUI implements Runnable, MessageListener {
	...
	public GameGUI() throws NamingException, JMSException {
		jmsHelper = new JMSHelper('localhost');
		try {
	...
	private JFrame mainframe;                 // login page
	private JFrame registerFrame;             // register page
	private JFrame gameFrame;                 // game page
	private MyPanel panel;                    
	private MyTable table;                    // the table of rank list 
	// word count to check whether there is some empty in the input
	private int wordCount = 0;                // name
	private int wordCount2 = 0;               // password
	private int wordCount3 = 0;               // register name
	private int wordCount4 = 0;               // register password
	private int wordCount5 = 0;               // fonfirm password
	...
	private int index = -1;                   // present page id.  0 for user profile, 1 for game page
	...
	private ArrayList<GamePlayer> leaderList; // leader list for rank page, max_num is 10
	...
	private Image[] cards;                    
	private int fromSource;                   // 0 >> login page, 1 >> register page
	private double expressionValue = -1;      // the value of the expression you submitted
	private boolean inGame = false;           // is in game or in the watting stage
	...
	private class MyActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			if (source.equals("login")) {
				...
				sendMessage(gameMessage);
				fromSource = 2;  //request is from the user profile page
				...
				if (JOptionPane.showConfirmDialog(mainframe, "Are you sure to log out?", "LOGOUT", 
	...
	public void run() {
		...
		mainframe.addWindowListener(new java.awt.event.WindowAdapter(){
		    ...
		    public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				int choice = JOptionPane.showConfirmDialog(mainframe, "Are you sure to close and exit?", "Close the window", 
			            JOptionPane.YES_NO_OPTION,
		...
		gameFrame.addWindowListener(new java.awt.event.WindowAdapter(){
			@Override
		    public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				int choice = JOptionPane.showConfirmDialog(mainframe, "Are you sure to leave the game?", "Close the window", 
			            JOptionPane.YES_NO_OPTION,
			            JOptionPane.QUESTION_MESSAGE);
		        if (choice == JOptionPane.YES_OPTION) {
		        	if (inGame) {
		...
		String rule = "JPoker 24-Game Rule:\n There will be max four players in this game.\n"
				+ "If ten seconds has been passed since the first player come in, game will start for any number of player more than 2 . \n"
				+ "Your goal is to be the quickest person to combine the 4 cards number to 24.\n You could use '*' '-' '+' '(' ')''. \n"
				+ "The game will be over if one of the four players submit the right answer.\n "
				+ "ATTENTION! : you can ONLY use A, J, Q, K to represent and try to keep them in front.\n"
